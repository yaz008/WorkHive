...
@router.add(
    name=FSAState.OwnerPointCity,
    pipeline=FSAPipeline.Owner,
    transitions={
        FSASymbol.Back: FSAState.OwnerPointAddress,
        FSASymbol.InputData: FSAState.OwnerPointCity,
        FSASymbol.Next: FSAState.OwnerPointPayload,
    },
    accepts_types=('text',),
)
def owner_point_city(
    owner: Owner, factory: ButtonFactoryClosure, city: str = str()
) -> TGMessage:
    point: TempPoint = TempPoint(owner.telegram_id)
    if city != str():
        point.city = find_closest(city)
    return TGMessage(
        text=render_file(
            ...
            tag_handlers={
                ...
                'city': (
                    lambda placeholder: f'<code>{(
                        point.city if point.city != str() else placeholder
                    )}</code>'
                ),
            },
        ),
